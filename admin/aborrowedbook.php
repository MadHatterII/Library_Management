...
<body>
    ...
    $totalRecords = $conn->query("
        SELECT COUNT(*) AS total FROM BorrowedBooks
    ")->fetch_assoc()['total'];
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Add the 'disabled-btn' class to disable the click
            $disabledClass = empty($row['ReturnDate']) ? '' : 'disabled-btn';
            
            echo "<td><button class='{$buttonClass} {$disabledClass}' onclick='returnBook({$row['BookID']})'>{$buttonText}</button></td>";
            echo "</tr>";
        }
            ...
            if (confirmReturn) {
                ...
                xhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        returnButton.classList.add('disabled-btn'); // Disable the button after returning
                    }
                };
